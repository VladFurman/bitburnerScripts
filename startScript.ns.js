let BASIC_ATTACK_SCRIPT = 'foodnstuff-hack.ns';
...
export async function main (ns) {
  ...
  if (threads > 0) {
    ns.run(BASIC_ATTACK_SCRIPT, getThreads('home', ns) - 10, 'start0');
  }
...
function getThreads (target, ns) {
  ...
  let freeRam = serverRamData[0] - serverRamData[1];
    // log(freeRam);
  let attackScriptRam = ns.getScriptRam(BASIC_ATTACK_SCRIPT);
